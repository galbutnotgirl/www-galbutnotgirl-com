    ...
    try {
        ...
        async.waterfall([
            ...
            function (data, callback) {
                if (!remove) {
                    var assetsIds = helper.getAssetsIds(data),
                        _assetsIds = [];
                    if (assetsIds && assetsIds.length) {
                        assetsIds = _.uniq(assetsIds);
                        var limit = 100,
                            totalRequests = Math.ceil(assetsIds.length/limit),
                            _requests = [];
                        for(var i = 0;i < totalRequests; i++){
                            _requests.push(function(i) {
                                return function(_cb) {
                                    request({
                                            url: api.host + api.urls.assets,
                                            headers: headers,
                                            method: "POST",
                                            qs: {limit: limit, desc: "updated_at"},
                                            json: {
                                                "_method": "GET",
                                                "query": { "uid": { "$in": assetsIds.slice((i*limit), (i*limit) + limit)}}
                                            }
                                        }, function(err, res, body){
                                            if(!err && res.statusCode == 200 && body && body.uploads && body.uploads.length){
                                                _assetsIds = _assetsIds.concat(body.uploads);
                                                _cb();
                                            }else{
                                                _cb(body);
                                            }
                        ...
                        async.series(_requests, function(err){
                            if(!err){
                                if(_assetsIds.length == assetsIds.length) {
                                    var _assets = {};
                                    for (var i = 0, _i = _assetsIds.length; i < _i; i++) {
                                        _assets[_assetsIds[i].uid] = (function (i) {
                                            return function (cb) {
                                                helper.getAssets(_assetsIds[i], lang, false, cb);
                                            }
                                        })(i);
                                    }
                                    async.series(_assets, function (_err, _data) {
                                        if (_err) {
                                            callback(_err, null);
                                        } else {
                                            data.entry = helper.replaceAssetsUrl(_data, data.content_type, data.entry);
                                            callback(null, data);
                                        }
                                    });
                                } else {
                                    _assetsIds = _.pluck(_assetsIds, "uid");
                                    callback(helper.message(new Error("Could not locate the following assets: [" + _.difference(assetsIds, _assetsIds).join(', ')) + "]."), null);
                                }
                            } else {
                                callback(err);
                            }
            ...
            function (data, callback) {
                ...
                helper.executePlugins(data.entry, data.content_type, lang, _action, callback);
            }
            ...
            try {
                    ...
                    if (!remove) {
                        var upsert = function() {
                            ...
                            db.ContentType(ctUID)
                                ...
                                .upsert(result.entry)
                                .then(function(data){
                                    status(entry, {"status": 2, "status_label": "Publish", "message": "Entry has been published successfully."}, self.next);
                                }, function(err){
                                    status(entry, {"status": 3, "status_label": "Fail", "message": "Error: " + eventText + "ing failed with the error(s): " + helper.message(err)}, self.next);
                                });
                        };
                        db.ContentType(contentTypesUid)
                            .language(lang.code)
                            ...
                            .then(function(data){
                                if(!data || (data.updated_at && data.updated_at !== result.content_type.updated_at)) {
                                    db
                                        ...
                                        .language(lang.code)
                                        .Entry(ctUID)
                                        .upsert(result.content_type)
                                        .then(function() {
                                            var _content_type = result.content_type;
                                            if(!data || (!data.options && !_content_type.options) || (data.options && _content_type.options && data.options.url_pattern === _content_type.options.url_pattern)) {
                                                upsert();
                                            } else if(!_content_type.singleton && _content_type.options && ((!data.options && _content_type.options.url_pattern) || (data.options && data.options.url_pattern !== _content_type.options.url_pattern))) {
                                                async
                                                    .parallel({
                                                            ...
                                                            db.ContentType("_routes")
                                                                ...
                                                                .then(function(entries) {
                                                                    ...
                                                                    db.ContentType("_routes")
                                                                        ...
                                                                        .Entry()
                                                                        .Query()
                                                                        .query({"_bulk_insert": true, "entries": entries})
                                                                        .then(function() {
                                                                            bulkCallback();
                                                                        }, function(err) {
                                                                            bulkCallback({"status": 3, "status_label": "Fail", "message": "Error: " + eventText + "ing failed with the error(s): " + helper.message(err)});
                                                                        });
                                                                }, function(err) {
                                                                    bulkCallback({"status": 3, "status_label": "Fail", "message": "Error: " + eventText + "ing failed with the error(s): " + helper.message(err)});
                                                                });
                                                            ...
                                                            db.ContentType(ctUID)
                                                                ...
                                                                .then(function(ctEntries) {
                                                                    ...
                                                                    db.ContentType(ctUID)
                                                                        ...
                                                                        .Entry()
                                                                        .Query()
                                                                        .query({"_bulk_insert": true, "entries": ctEntries})
                                                                        .then(function() {
                                                                            bulkCallback();
                                                                        }, function(err) {
                                                                            bulkCallback({"status": 3, "status_label": "Fail", "message": "Error: " + eventText + "ing failed with the error(s): " + helper.message(err)});
                        ...
                        db.ContentType(ctUID)
                            ...
                            .then(function(data){
                                ...
                                status(entry, {"status": 3, "status_label": status_label, "message": "Error: " + eventText + "ing failed with the error(s): " + helper.message(err)}, self.next);
                            });
                    }
                } else {
                    if (error.stack)
                ...
                status(entry, {"status": 3, "status_label": "Fail", "message": "Error: " + eventText + "ing failed with the error(s): " + e.message}, self.next);
            }
        });
    } catch (e) {
            ...
            function (callback) {
                if (!remove) {
                    if (assetUID) {
                        ...
                        request.get({url: _url, headers: headers, json: true}, function (err, res, body) {
                            if (!err && res.statusCode == 200) {
                                if (body.upload) {
                                    body.upload.force_load = false;
                                    helper.getAssets(body.upload, lang, false, callback);
                                } else {
                    ...
                    helper.getAssets(assetUID, lang, true, callback);
                }
            }
        ], function (error, data) {
            try {
...
Sync.prototype.form = function (languages, form) {
    ...
    try {
        ...
        function (err, data) {
            if(err) {
                status(form, {"status": 3,  "status_label": "Fail", "message": "Error: Deleting failed with the error(s): " + helper.message(err)}, self.next);
            } else {
                status(form, {
                    "status": "4",
                    "status_label": "Delete",
