                ...
                calls[languages[l]['code']] = (function (language) {
                    return function (cb) {
                        ...
                        if(fs.existsSync(model)) {
                            ...
                            // setting content_types and entry routes in the memory
                                            ...
                                            return function (_cb) {
                                                fs.readFile(filePath, 'utf-8', function (err, data) {
                                                    if (err) {
                                                        ...
                                                        _cb(null, helper.filterEntries(form.uid, self._inmemory[form.uid], JSON.parse(data)));
                                                    }
                                                });
                                            }
...
Inmemory.prototype.get = function(language, content_type_id, query) {
    var result;
...
Inmemory.prototype.set = function(language, content_type_id, uid, data, partial) {
    if(language && content_type_id) {
        if(this.cache[language]) {
            ...
            if(content_type_id === contentTypeName || content_type_id === entryRoutesName) {
            ...
            } else if((this._inmemory[content_type_id] && this._inmemory[content_type_id].length) || partial || (this.cache[language][content_type_id])) {
                ...
                var idx;
                if(uid) {
                    idx = _.findIndex(this.cache[language][content_type_id], {"_uid": uid});
                    if(~idx) this.cache[language][content_type_id].splice(idx, 1);
                    if(data) this.cache[language][content_type_id].unshift(data);
                } else {
                    this.cache[language][content_type_id] = data;
