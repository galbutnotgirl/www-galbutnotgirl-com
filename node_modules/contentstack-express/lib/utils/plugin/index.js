...
plugin.load = function() {
	...
	if(typeof __plugins === 'object' && Object.keys(__plugins).length) {
		...
		for(var plugin_key in __plugins) {
			...
			if(plugin_key && typeof plugin_key === 'string' && pluginOptions && (fs.existsSync(path.join(_path, plugin_key, 'index.js')) || fs.existsSync(inBuiltPluginsPath, plugin_key, 'index.js'))) {
				var currentPlugin;
				...
				if(typeof currentPlugin === 'function') {
					...
					throw new TypeError(plugins[i] + ' should be a function.');
				}
			}
		}
	}
